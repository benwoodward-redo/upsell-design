...
// Create 8x8 Bayer Matrix Data Texture
...
const bayerDataRGBA = new Uint8Array(64 * 4);
for (let i = 0; i < 64; i++) {
  const val = Math.floor(bayerMatrix8x8[i] * 255);
  bayerDataRGBA[i * 4] = val;
  bayerDataRGBA[i * 4 + 1] = val;
  bayerDataRGBA[i * 4 + 2] = val;
  bayerDataRGBA[i * 4 + 3] = 255;
}
...
vec3 dither(vec2 uv, vec3 color) {
  // Use texture-based bayer matrix
  vec2 coord = uv * resolution / pixelSize;
  vec2 bayerUV = coord / 8.0; 
...
void main() {
  ...
  // Calculate pixelated UV for the wave function to simulate low-res rendering
  vec2 normalizedPixelSize = pixelSize / resolution;
  vec2 pixelatedUV = normalizedPixelSize * floor(uv / normalizedPixelSize);
  
  // Wave Logic
  vec2 waveUv = pixelatedUV;
  waveUv -= 0.5;
...
function DitheredWaves({
  ...
  const waveUniformsRef = useRef({
    ...
    waveColor: new Uniform(
      new Color(waveColor[0], waveColor[1], waveColor[2]),
    ),
    backgroundColor: new Uniform(
      new Color(
        backgroundColor[0],
        backgroundColor[1],
        backgroundColor[2],
    ...
    enableMouseInteraction: new Uniform(
    ...
    bayerTexture: new Uniform(bayerTexture),
  });
  ...
  useFrame(({ clock }) => {
    ...
    u.backgroundColor.value.set(
      backgroundColor[0],
      backgroundColor[1],
      backgroundColor[2],
    );

    u.enableMouseInteraction.value = enableMouseInteraction
      ? 1
      : 0;
...
export default function Dither({
  ...
  backgroundColor = [0.976, 0.98, 0.984],
  colorNum = 8,
  pixelSize = 2,
